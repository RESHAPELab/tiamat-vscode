...
export async function syncPersonalization(context: vscode.ExtensionContext) {
    try {
        ...
        await config.update('tiamat.personalizedPrompt', personalization.personalizedPrompt.split("\n"), vscode.ConfigurationTarget.Global);
...
export async function updatePersonalization(context: vscode.ExtensionContext, newPersonalization: string[]) {
    try {
        ...
        await put(`${apiUrl}/api/personalization/${userId}`, { personalization: { personalizedPrompt: newPersonalization.join("\n") } });
